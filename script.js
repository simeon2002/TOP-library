...
class App {
  ...
  #loadLibrary() {
    return (
      this.#fetchFromLocalStorage("library")?.map(book => new Book(book.title, book.author, book.pages, book.readStatus, book.bookId)) ?? []
    );
  }
  ...
  displayBooks(displayFormat = "table") {
    ...
    if (displayFormat === "table") {
      html =
        this.#isLibEmpty() === false
          ? this.library.map(book => this.#getBookHtml(book, displayFormat)).join("")
